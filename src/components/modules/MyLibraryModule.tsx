...
export const MyLibraryModule: React.FC<MyLibraryModuleProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {settings && (
        <div className="p-5 rounded-2xl border border-emerald-500/30 bg-white space-y-3 shadow-lg">
          <div className="flex items-center justify-between border-b border-slate-200/90 pb-3">
            ...
            <span className="text-[10px] text-slate-500 font-mono bg-[#f1f5f9] px-2 py-0.5 rounded border border-slate-200/90">
              {settings.institutionName}
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            <div className="flex items-start space-x-2.5 p-2.5 rounded-xl bg-[#f1f5f9] border border-slate-200/90">
              <MapPin className="h-4 w-4 text-red-400 shrink-0 mt-0.5" />
              <div>
                <span className="text-[10px] text-slate-500 block font-medium">Place & Location</span>
                <span className="text-white font-bold block">{settings.libraryPlace || 'Main Campus Central Building'}</span>
                <span className="text-slate-500 text-[11px]">{settings.libraryLocation || 'Pakistan'}</span>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-2.5 p-2.5 rounded-xl bg-[#f1f5f9] border border-slate-200/90">
              ...
              <div>
                <span className="text-[10px] text-slate-500 block font-medium">Mobile & Office Telephones</span>
                <span className="text-white font-bold block">{settings.libraryMobileNo || '+92 300 9876543'}</span>
                <span className="text-slate-500 text-[11px]">{settings.libraryOfficeNo || '+92 51 92654321'}</span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 rounded-2xl border border-slate-200/90 bg-white space-y-3">
          ...
          {myLoans.length === 0 ? (
            <p className="text-xs text-slate-500">No active book loans at present.</p>
          ) : (
            myLoans.map(l => (
              <div key={l.id} className="p-3 rounded-xl border border-slate-200/90 bg-[#f1f5f9] text-xs space-y-1">
        ...
        <div className="p-6 rounded-2xl border border-slate-200/90 bg-white space-y-3">
          ...
          {myHolds.length === 0 ? (
            ...
            myHolds.map(h => (
              <div key={h.id} className="p-3 rounded-xl border border-slate-200/90 bg-[#f1f5f9] text-xs space-y-1">
                <div className="font-bold text-slate-900">{h.bookTitle}</div>
                <div className="text-[10px] text-slate-500 flex items-center justify-between">
